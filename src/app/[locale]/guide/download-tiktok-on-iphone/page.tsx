...
const SLUG = '/guide/download-tiktok-on-iphone'

type Props = { params: Promise<{ locale: string }> }

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'GuideIphone' })
...
export default async function DownloadTikTokOnIphone({ params }: Props) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: 'GuideIphone' })
  const localePath = locale === 'en' ? '' : `/${locale}`
  ...
  const jsonLd = [
    ...
    },
    {
      ...
      name: t('h1'),
      totalTime: 'PT1M',
      tool: [{ '@type': 'HowToTool', name: 'iPhone with Safari' }, { '@type': 'HowToTool', name: 'TokDown (tokdown.org)' }],
        ...
        <main className="relative z-10 flex flex-col items-center w-full px-4 py-12">
          ...
          <header className="w-full max-w-2xl mb-10 text-center">
            <p className="text-xs font-semibold mb-3 tracking-widest uppercase" style={{ color: '#ff2d55' }}>
              {locale === 'ar' ? 'دليل آيفون · يونيو 2026' : locale === 'de' ? 'iPhone-Anleitung · Juni 2026' : 'iPhone Guide · June 2026'}
            </p>
          ...
          <section className="w-full max-w-2xl mb-4" aria-labelledby="related-h">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                { href: `${localePath}/guide/download-tiktok-on-android`, icon: '🤖', en: 'Download on Android', de: 'TikTok auf Android', ar: 'تنزيل على أندرويد', desc_en: 'Save TikTok videos to your Android gallery.', desc_de: 'TikTok-Videos in Android-Galerie speichern.', desc_ar: 'احفظ فيديوهات تيك توك في معرض أندرويد.' },
